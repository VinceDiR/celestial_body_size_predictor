...
data = pd.read_csv(
    path, storage_options={"token": credentials}, low_memory=False, index_col=0
)
...
st.header(
    "Let's see if we can predict the diameter of asteroids using Gradient Boosted Regression"
)
st.subheader(
    """The Data, courtesy of [NASA's Jet Propulsion Laboratory](https://ssd.jpl.nasa.gov/tools/sbdb_query.html)"""
)
...
with col3:
    st.metric(label="Train Set", value=0.75)
with col4:
    st.metric(label="Test Set", value=0.25)


model_data = data2.drop(
    columns=[
...
model_data = pd.get_dummies(
    model_data, prefix="class", columns=["Asteroid Orbit Class"]
)
model_data = pd.get_dummies(
    model_data, prefix="condition_code", columns=["Condition Code"], drop_first=False
)
